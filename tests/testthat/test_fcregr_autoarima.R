...
test_that("fcregr_auto.arima", {
  ...
  parset.list[[4]]$h = 1L
  parset.list[[5]]$h = 1L
  testSimpleParsets("fcregr.auto.arima", fcregr.df, fcregr.target,
...
test_that("fcregr_auto.arima_update", {
  ...
  for (i in seq_len(length(parset.list))) {
    ...
    parset$model = m
    pars.update = list(y = as.ts(fcregr.update.update$test_data), model = m)
  ...
  parset.list[[4]]$h = 1L
  parset.list[[5]]$h = 1L
  testSimpleParsetsUpdate("fcregr.auto.arima", fcregr.update.df, fcregr.target,
